...
global v_y;
    ...
    % where is a specific luminance L? It's at
    % vals*displayrange+displayBasline
    ...
    secondFit = fittedmodel(linspace(0,1,1000000));
    % first, find the input level where the output is 100.
    
    % try to convert back
    x = inputV*maxLevel;
    y = vals*displayRange+displayBaseline;
    
    
    % where is y = 100?
    norm_x = linspace(0,1,1000000);
    unnorm_x = norm_x*maxLevel;
    
    norm_y = secondFit;
    unnorm_y = secondFit*displayRange+displayBaseline;
    plot(unnorm_x,unnorm_y)
    
    ind100 = find(unnorm_y>100,1);
    unnorm_y(ind100);
    input_100 = unnorm_x(ind100);
    max_norm_y = norm_y(ind100);
    max_norm_x = norm_x(ind100);

    % ok, now we want to evaluate the model from 0 max
    
    gammaTable2 = (linspace(0,max_norm_x,256)).^(fittedmodel.g); %#ok<NBRAK>
    gamma_vals_normed = gammaTable2/max_norm_y;
    plot(linspace(0,1,256),gamma_vals_normed);
    newfit2 = fit(linspace(0,1,256)',gamma_vals_normed',g);
    figure; plot(linspace(0,1,100),linspace(0,1,100).^(1/newfit2.g));
    gammaTable3 = linspace(0,1,256).^(1/newfit2.g);
    gammaTable3 = gammaTable3 * max_norm_y;
    
    plot(linspace(0,1,256),gammaTable3);hold on;plot(gamma_vals_normed,linspace(0,1,256)*max_norm_y);

    
    gammaTable = gammaTable3';
    save MyGammaTable gammaTable
return;
