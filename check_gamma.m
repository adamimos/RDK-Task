function [ gammaTable1, gammaTable2, displayBaseline, displayRange, displayGamma, maxLevel ] = check_gamma(numMeasures, screenid)
...
global inputV;
    ...
    gammaTable2 = fittedmodel([0:maxLevel]/maxLevel); %#ok<NBRAK>
        ...
        % Adam Shai - make gamma table that goes from 0 to 100 cd/m^2
 
    xxx = 0:255;
    figure
    plot(xxx,displayRange*(xxx/maxLevel).^(displayGamma) + displayBaseline)
    
    % find where gamma = 100
    
    x100 = maxLevel* ((100 -displayBaseline) / displayRange) .^(1/displayGamma);
     
   % so we want the table to be from 0 = baseline to 255 = x100, following
   % this curve, so:
   
   xxx = linspace(0,x100,maxLevel+1);
    plot(xxx,displayRange*(xxx/maxLevel).^(displayGamma) + displayBaseline)
    
    gammaTable3 = (xxx/maxLevel).^(1/displayGamma);
    gammaTable3 = gammaTable3';
