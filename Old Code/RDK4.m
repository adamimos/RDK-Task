    ...
    while didRespond == 0
        %% show stim
            ...
            if test.is_licking(2)
                ...
                %if response.timeVis(trial)>0.25
...
tVis = response.timeVis(response.timeVis>0);
mintVis = min(tVis);
maxtVis = max(tVis);
plot(linspace(mintVis,maxtVis,10),histc(tVis(rrr==0),linspace(mintVis,maxtVis,10)),linspace(mintVis,maxtVis,10),histc(tVis(rrr==1),linspace(mintVis,maxtVis,10)),'parent',handles.RTAll);
