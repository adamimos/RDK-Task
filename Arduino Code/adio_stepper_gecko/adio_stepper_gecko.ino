...
const int lickPort1Pin = 26;
const int lickPort2Pin = 36;
const int lickPort3Pin = 46;

const int nVistaTTL = 12;
const int nVistaInterupt = 18;
...
void setup() {
  /* initialize serial                                       */
  pinMode(lickPort1Pin, INPUT);
  pinMode(lickPort2Pin, INPUT);
  pinMode(lickPort3Pin, INPUT);
  pinMode (encoder0PinA,INPUT_PULLUP);
  pinMode (encoder0PinB,INPUT_PULLUP);
  pinMode(nVistaTTL,OUTPUT);
  attachInterrupt(digitalPinToInterrupt(nVistaInterupt), frame_counter, RISING);
  Serial.begin(115200);
...
void loop() {
 ...
 n = digitalRead(encoder0PinA);
   ...
   encoder0PinALast = n;
  digitalWrite(lickometerOut, !digitalRead(lickPort2Pin));

  /* variables declaration and initialization  */

  instalick1 = digitalRead(lickPort1Pin);
  ...
  if (Serial.available() > 0) {
    ...
    switch (s) {
      ...
      case -1:
        ...
        if ((s > 40 && s < 90) || (s > 90 && s != 340 && s != 400 && s != 410 && s!= 200 && s !=210 && s !=100 && s!=300 && s!= 270 && s != 160 && s!=170)) {
          s = -1;
        }
      ...
      case 100:
        Serial.println(lickPort1State);
        s=-1;  /* we are done with the aux function so -1      */
        break; /* s=412 taken care of                          */
        
      case 200:
        /*Serial.println(encoder0Pos);
        s = -1;
        break;*/
        Serial.println(lickPort2State);
        s = -1;
        break;

      case 300:
        Serial.println(lickPort3State);
        s=-1;  /* we are done with the aux function so -1      */
        break; /* s=412 taken care of                          */
